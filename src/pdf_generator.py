...
def generate_report(
    ...
    if leads:
        ...
        # Table header — includes buyer contact info
        header = ["#", "Company", "Industry", "Contact / Role", "Website", "Score", "Why They Need PerfectParser"]
        col_widths = [
            0.03 * doc.width,   # #
            0.16 * doc.width,   # Company
            0.11 * doc.width,   # Industry
            0.16 * doc.width,   # Contact / Role
            0.16 * doc.width,   # Website
            0.07 * doc.width,   # Score
            0.31 * doc.width,   # AI Reason
        ]

        table_data = [header]

        for idx, lead in enumerate(leads, start=1):
            # Build "Contact / Role" cell — who would buy this product
            ...
            row = [
                ...
                Paragraph(contact_cell, styles["cell"]),
                Paragraph(lead.get("website") or lead.get("profile_url") or "N/A", styles["cell"]),
                Paragraph(
                    f'<font color="{_score_color(lead.get("lead_score") or "").hexval()}">'
                    f'<b>{lead.get("lead_score") or "N/A"}</b></font>',
                    styles["cell"],
                ),
                Paragraph(lead.get("ai_reason") or lead.get("reason") or "N/A", styles["cell"]),
            ]
